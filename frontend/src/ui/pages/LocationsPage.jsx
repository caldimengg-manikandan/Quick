...
import { Calendar, ChevronLeft, ChevronRight, MapPin, Clock, MousePointer, Monitor, Image as ImageIcon, CheckCircle, Navigation, Activity, Camera } from "lucide-react"
...
export function LocationsPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 110px)", width: "100%", backgroundColor: "var(--bg)", border: "1px solid var(--stroke)", borderRadius: "var(--radius-lg)", overflow: "hidden", boxShadow: "var(--shadow)" }}>
      {/* Top Header */}
      <div style={{ padding: "12px 24px", display: "flex", gap: "12px", borderBottom: "1px solid var(--stroke)", alignItems: "center", backgroundColor: "var(--surface)" }}>
        <button 
        ...
        <div style={{ flex: 1, position: "relative", backgroundColor: activeTab === 'locations' ? "transparent" : "var(--surface)", display: "flex", flexDirection: "column" }}>
          ...
          {activeTab === 'locations' && (
            <MapContainer 
              ...
              zoomControl={true}
              key={mapCenter.join(',')} // Retriggers map when center changes entirely for first load
            >
              <TileLayer
